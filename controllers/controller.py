...
def save_game(players:dict, game_status:list, user_input: list) -> None:
    list_to_dict:dict = {}
    file_name: str = user_input[1]

    save_game_file:open = open(file_name, "w")
...
def load_saved_game(user_input: list) -> None:
    load_data:list = []
    players:dict = {}
    game_status:list = []
    file_name: str = user_input[1]

    with open(file_name, encoding = "utf-8") as save_game_file:
    ...
    if is_safe_to_make_move == 1:
        ...
        while cpu_player_keep_play == 1:  
            ...
            if game_status[1] == "Normal":
                ...
                board, keep_playing, game_is_finished = play.normal_ai_play(player1_or_player2, board)
                game_status[3] = board

            elif game_status[1] == "Avançado":
                player1_or_player2 = ver.verify_if_player1_or_2(game_status[1], game_status)
                board, keep_playing, game_is_finished = play.advanced_ai_play(player1_or_player2, board)
                game_status[3] = board
